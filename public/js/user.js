...
window.addEventListener('DOMContentLoaded', (event) => {
  ...
  fetch('/listUserProducts/' + JSON.parse(localUser).Id, { method: 'GET' })
    ...
    .then(function (data) {
      ...
      for (let product in data) {
        ...
        producthtml += `<p class="mb-0">${data[product].Price}</p>`;
  ...
  fetch('/followedProducts/' + JSON.parse(localUser).Id, { method: 'GET' })
    .then(function (response) {
      if (response.ok) {
        console.log(response);
        return response.json();
    ...
    .then(function (data) {
      console.log(data);
      const productshtml = document.getElementById('followedProducts');
      let producthtml = '';
      for (let product in data) {
        producthtml += `<div id="product" class="p-${product.Id}">`;
        producthtml += `<h3>${data[product].Title}</h3>`;
